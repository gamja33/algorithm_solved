...
(사용한 ESP32S3 보드 모델에 따라 다를 수 있으나) DAC (Digital-to-Analog Converter)가 내장되지 않은 핀을 tone()으로 사용하면, PWM 방식(사각파)으로 소리를 생성합니다.

이 경우 음질이 맑지 않을 수 있으나, 소리를 알아듣는 데는 큰 문제가 없습니다.

DAC 없이 음질 향상 (라이브러리)

XT_DAC_Audio 라이브러리를 사용하면, DAC 핀이 없어도 PWM을 통해 더 나은 품질의 오디오(WAV 등)를 재생할 수 있습니다.
...
🚀 향후 계획 (Next Step)
작업이 빨리 끝난다면, 라즈베리파이에서 **Classic BT (블루투스)**로 오디오 파일을 (ESP32 스피커로) 전송하는 기능을 구현합니다.

동시에 버튼 입력을 통해 **BLE (저전력 블루투스)**로 스마트폰과의 거리를 감지하는 기능을 추가합니다.
